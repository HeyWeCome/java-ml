    ...
    $.ajax({
        ...
        success: function (userDairy) {
            ...
            for (var i in userDairy){
                var oneDiary =
                    "<div class=\"userDiary\" id=\""+userDairy[i].id+"\">" +
                    "<h6 class=\"diaryTime\">\n" +
                    ...
                    userDairy[i].content
                    +"</p><button type=\"button\" class=\"btn btn-dark deleteDiary\" id=\""+userDairy[i].id+"button\">" +
                    "" +
                    "删除</button></div>";

                $("#diaryMain").append(oneDiary);
                $("#"+userDairy[i].id+"button").bind("click",function () {
                    deleteDiary(userDairy[i].id);
                });
            }
        },
        error: function () {
